...
overlap = [8,16,32]
filters = [32,64] 
batches = [1024, 512, 256, 128, 64]
lr = [5e-4, 2.5e-4, 1e-4, 7.5e-5, 5e-5]
...
for i in range(len(patches)):
    ...
    CHECKPOINTS_DIR = Path(f"D:\Funmilayo_data\model_weights{i}")  #Path.cwd() / "model_weights"
    CHECKPOINTS_DIR.mkdir(parents=True, exist_ok=True)
    TENSORBOARD_LOGDIR = CHECKPOINTS_DIR #"D:/Funmilayo_data/runs" 
    TEXT_DIR = CHECKPOINTS_DIR/"parameters.txt"
    
    # training settings and hyperparameters
    NO_VALIDATION_PATIENTS = 25
    ...
    unet_model = u_net.AttentionNet(num_classes=1, nr_filters=32).to(device)
    ...
    unet_model.eval()
    for i in range(10):
        valid_dataset = utils.ValidDataset(partition["validation"], IMAGE_SIZE, PATCH_SIZE, STRIDE,i, 25) 
        
        # load model
        unet_model = u_net.AttentionNet().to(device)
        unet_model.load_state_dict(torch.load(CHECKPOINTS_DIR2), strict=False)
        unet_model.eval()
        ...
        with torch.no_grad():
            ...
            plt.savefig(CHECKPOINTS_DIR / f"img{i}.png" , format='png')   # save the figure to file
            plt.close(fig) 
